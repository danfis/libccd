...
TEST(mprBoxboxAlignedX)
{
    ...
    for (i = 0; i < 100; i++){
        res = ccddblMPRIntersect(&box1, &box2, &ccddbl);
        if (i < 35 || i > 65){
            assertFalse(res);
        }else if (i != 35 && i != 65){
            assertTrue(res);
...
TEST(mprBoxboxPenetration)
{
    ...
    /*
    ccddblVec3Set(&box2.pos, 0., 0., 0.);
    res = ccddblMPRPenetration(&box1, &box2, &ccddbl, &depth, &dir, &pos);
    assertTrue(res == 0);
    ...
    */
    ...
    ccddblVec3Set(&box1.pos, 0., 0., 0.);
    ccddblQuatSet(&box1.quat, 0., 0., 0., 1.);
    ccddblVec3Set(&box2.pos, 0., 0.73, 0.);
    ccddblQuatSet(&box2.quat, 0., 0., 0., 1.);
    ...
    ccddblVec3Set(&box1.pos, 0., 0., 0.);
    ccddblQuatSet(&box1.quat, 0., 0., 0., 1.);
    ccddblVec3Set(&box2.pos, 0.3, 0.738, 0.);
    ccddblQuatSet(&box2.quat, 0., 0., 0., 1.);
